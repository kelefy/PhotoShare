...
//    [self.editor fixZoomScaleWithAnimated:YES];
    ...
    _menuScroll = [[UIScrollView alloc] initWithFrame:CGRectMake(0, self.editor.view.frame.size.height-80, self.editor.view.frame.size.height, 80)];//self.editor.menuView.frame];
...
//     _workingView = [[UIView alloc]initWithFrame:self.editor.imageView.frame
    ...
    _settingView = [[CLTextSettingView alloc] initWithFrame:CGRectMake(0, self.editor.view.frame.size.height-180-80, self.editor.view.width, 180)];
    [_settingView setSelectedFillColor:[UIColor whiteColor]];
//    _settingView.top = _menuScroll.top - _settingView.height;
    _settingView.backgroundColor = [UIColor colorWithWhite:1 alpha:0.8];//[CLImageEditorTheme toolbarColor];
    _settingView.textColor = [UIColor blackColor];//[CLImageEditorTheme toolbarTextColor];
...
//    [_workingView addSubview:view];
    ...
    CGFloat ratio = MIN( (0.8 * _workingView.width) / view.width, (0.2 * _workingView.height) / view.height);
    [view setScale:1];
    view.center = CGPointMake(_workingView.width/2, _workingView.height/2 - 50);
    
    
    [_workingView addSubview:view];
    [_CLTextView setActiveTextView:view];
    [self.editor.view bringSubviewToFront:_settingView];
...
//    [UIButton buttonWithFrame:CGRectMake(view.width-70, 0, 70, 70) imageName:@"TuSDK.bundle/ui_default/style_default_edit_button_confirm_bg"];
    ...
    UIView *fiterBar = nil;//[[UIView alloc]initWithFrame:CGRectMake(0, self.editor.view.frame.size.height-49-80, self.editor.view.frame.size.width, 49)];
    fiterBar.backgroundColor = [UIColor colorWithRed:0.1176470588 green:0.1098039216 blue:0.1215686275 alpha:1.0000000000];
    [self.editor.view addSubview:fiterBar];
    
    CGFloat W = 70;
    CGFloat H = _menuScroll.height;
    
    CGFloat space = (self.editor.view.frame.size.width-12-240)/3;
    CGFloat x = space+20+50;
    NSInteger margin = 25;
    UIButton *cancel = [UIButton buttonWithType:UIButtonTypeCustom];
    cancel.frame = CGRectMake(margin, 20, 40, 40);
    [cancel setImage:[UIImage imageNamed:@"TuSDK.bundle/ui_default/style_default_edit_button_cancel"] forState:UIControlStateNormal];
    [cancel addTarget:self.editor action:@selector(pushedCloseBtn:) forControlEvents:UIControlEventTouchUpInside];
//    [_menuScroll addSubview:cancel];
    
    
    
    NSArray *_menu = @[
//  @{@"title":@" ", @"icon":[UIImage imageNamed:@"TuSDK.bundle/ui_default/style_default_edit_button_cancel"]}
//                       ,
                       @{@"title":[CLImageEditorTheme localizedString:@"CLTextTool_MenuItemNew" withDefault:@"New"], @"icon":[UIImage imageNamed:@"addText"]}
    ...
    for(NSDictionary *obj in _menu){
        CLToolbarMenuItem *view = [CLImageEditorTheme menuItemWithFrame:CGRectMake((self.editor.view.frame.size.width-212)/3+60, 15, 50, 50) target:self action:@selector(tappedMenuPanel:) toolInfo:nil];
        view.tag = tag++;
...
//                _textBtn = view;
                break;
            case 2:
                view = [CLImageEditorTheme menuItemWithFrame:CGRectMake(x, 24, 32, 32) target:self action:@selector(tappedMenuPanel:) toolInfo:nil];
                _colorBtn = view;
                _colorBtn.iconView.layer.borderWidth = 2;
                _colorBtn.iconView.layer.borderColor = [[UIColor blackColor] CGColor];
                view.tag = 2;
//                _colorBtn.frame = CGRectMake(x, 20, 40, 40);
                break;
            case 3:
//                _fontBtn = view;
                continue;
...
//        [fiterBar addSubview:view];
        [_menuScroll addSubview:view];
        x = (self.editor.view.frame.size.width-212)/3 + view.frame.origin.x + view.frame.size.width;
    }
    
    _menuScroll.bounces = NO;
    UIButton *complete = [UIButton buttonWithType:UIButtonTypeCustom];
    complete.frame = CGRectMake(self.editor.view.frame.size.width-margin-40, 20, 40, 40);
    [complete setImage:[UIImage imageNamed:@"TuSDK.bundle/ui_default/style_default_edit_button_completed"] forState:UIControlStateNormal];
    [complete addTarget:self.editor action:@selector(pushedFinishBtn:) forControlEvents:UIControlEventTouchUpInside];
//    [_menuScroll addSubview:complete];
    x+=40;
...
-(void)addButtonsToMenuScroll:(NSArray *)btnList
{
    NSInteger count = [btnList count];
    NSInteger insetWidth = 35;
    ...
    float spaceWidth = (float)(_menuScroll.frame.size.width-btnWithTotal)/(count - 1);
    ...
    for(int i=0;i<btnList.count;i++)
    {
        ...
        if(i==0)
        {
            btn.frame = CGRectMake(insetWidth,(_menuScroll.frame.size.height - btn.frame.size.height)/2,btn.frame.size.width,btn.frame.size.height);
        }
        else
        {
            UIButton *last = btnList[i-1];
            btn.frame = CGRectMake(last.frame.origin.x+last.frame.size.width+spaceWidth,(_menuScroll.frame.size.height - btn.frame.size.height)/2,btn.frame.size.width,btn.frame.size.height);
        }
...
const CGFloat MAX_FONT_SIZE = 25.0;
...
//        _label.outlineWidth = 200;
        ...
        _label.minimumScaleFactor = 3/MAX_FONT_SIZE;
...
//        [_deleteButton setImage:[tool imageForKey:kCLTextToolDeleteIconName defaultImageName:@"btn_delete.png"] forState:UIControlStateNormal];
        ...
        [_deleteButton setBackgroundColor:[UIColor orangeColor]];
        ...
        _circleView.color = [UIColor blueColor];
        _circleView.borderColor = [UIColor blackColor];
        _circleView.borderWidth = 1;
//        UIImageView *image = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"circle_blue"]];
        UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
        [btn setImage:[UIImage imageNamed:@"circle_blue"] forState:UIControlStateNormal];
        btn.frame = CGRectMake(0, 0, 32, 32);
        [_circleView addSubview:btn];
...
- (void)setAvtive:(BOOL)active
{
    ...
    _label.layer.borderWidth = (active) ? 1.5/_scale : 0;
}
...
- (void)setScale:(CGFloat)scale
{
    ...
    _label.layer.borderWidth = 1.5/_scale;
...
- (void)setText:(NSString *)text
{
    if(![text isEqualToString:_text]){
        _text = text;
        _label.text = (_text.length>0) ? _text : @"请输入文字";//[CLImageEditorTheme localizedString:@"CLTextTool_EmptyText" withDefault:@"Text"];
    }
}
...
- (void)circleViewDidPan:(UIPanGestureRecognizer*)sender
{
    ...
    [self setScale:MAX(_initialScale * R / tmpR, 6/MAX_FONT_SIZE)];
}
